...
export function NoiseStep({ graphId, optimalCutValue, bestPoint }: NoiseStepProps) {
  ...
  return (
    <section style={{ marginTop: 48, marginBottom: 64 }}>
      <h1>Step 8: ノイズありシミュレーションとの比較</h1>
      <p>
      ...
      <Callout label="補足：誤りの掛け算的な積み重なり">
        ...
        <p style={{ margin: 0, fontSize: 14, color: '#334155' }}>
          λが小さくてもゲート数が多いほどこの確率は下がります——Step 6・7で見た
          「層を増やすほど理論上は良くなる」という利点は、実機では回路が深くなる
          ぶんノイズにさらされる機会も増えるというトレードオフと表裏一体です。
          誤り率を上げていくと、測定結果は徐々にStep 2で見た一様分布（位相情報が
          確率に反映される前の状態）に近づいていきます——量子ビットの情報が
          壊れるほど、量子計算としての意味を失っていくということです。
        </p>
      </Callout>

      <Callout label="補足：なぜ「ノイズあり」側だけショット計測なのか">
        <p style={{ margin: 0, fontSize: 14, color: '#334155' }}>
          Step 2〜7で使ってきたStatevectorは、常に純粋状態|ψ⟩だけを厳密に追跡する
          理想シミュレータで、雑音（混合状態）を扱えません。そこでStep 8だけ
